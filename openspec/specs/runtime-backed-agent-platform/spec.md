# runtime-backed-agent-platform Specification

## Purpose
Defines the runtime-backed architecture boundary for Yunluostar, where the CLI owns user interaction, workspace context collection, local auth token storage, and local tool execution, while the runtime owns cognition, provider access, embedding, memory, goals, reflection, audit logs, and scoped persistence.
