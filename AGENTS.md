...
- **Bot Runtime**: 当前为 Node.js 原生 HTTP runtime；Hono 可作为后续可选重构方向，不是当前依赖
...
`episodes` (情节记忆) | `semantic_memories` (语义记忆) | `self_model` (自我模型) | `user_model` (用户模型) | `goals` (目标) | `working_memory_snapshots` | `reflections` (反思) | `audit_logs` (审计日志)
