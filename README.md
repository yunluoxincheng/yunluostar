...
| Bot Runtime | 现有 HTTP runtime + Hono |
...
yunluostar 正在从 CLI-first agent 研究原型转型为多平台认知型 Agentic Bot 平台。
...
- Runtime Bot Message Path — Bot 消息接入认知流水线
- HTTP Webhook Adapter — 第一个非 CLI Bot 入口
- WebChat-Compatible API — `POST /v1/bot/message`
- Plugin System MVP — 插件 manifest、registry、hooks
